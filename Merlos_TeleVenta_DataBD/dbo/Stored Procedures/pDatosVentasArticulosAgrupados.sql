...
CREATE PROCEDURE [dbo].[pDatosVentasArticulosAgrupados](@parametros VARCHAR(MAX))
AS
	...
	BEGIN TRY	
		BEGIN TRAN			
            ...
            SET @gestionDosAñosPasados= CONCAT('[',@ejerDosAñosPasados,@letraGestion,']')			
            ...
            IF EXISTS(SELECT name FROM sys.databases WHERE name = CONCAT(@ejerActual,@letraGestion)) AND EXISTS(SELECT name FROM sys.databases WHERE name = CONCAT(@ejerPasado,@letraGestion) AND NOT EXISTS (SELECT name FROM sys.databases WHERE name = CONCAT(@ejerDosAñosPasados,@letraGestion)))
                BEGIN
					...
					where asd.ARTICULO!='''' and b.TRASPEJER=0 and b.cliente=''' + @cliente + ''' and b.fecha between '''+ @fechaDesde+'''and  '''+@fechaHasta+'''
                    UNION ALL'
                    SET @cSQL1= @cSQL1 + ' SELECT  asd.ARTICULO, asd.precio, ROW_NUMBER() OVER(PARTITION BY asd.ARTICULO ORDER BY b.FECHA DESC) AS rn
					...
					INNER JOIN ' + @gestionActual + '.dbo.c_pedive b ON  b.EMPRESA = a.EMPRESA AND b.NUMERO = a.NUMERO AND b.LETRA = a.LETRA
					LEFT JOIN LatestPrices lp ON a.articulo = lp.ARTICULO AND lp.rn = 1
					WHERE A.ARTICULO!='''' and b.TRASPEJER=0 and  b.cliente=''' + @cliente + ''' and b.fecha between '''+ @fechaDesde+'''and  '''+@fechaHasta+'''
					GROUP BY a.articulo,a.DEFINICION, a.empresa,a.cliente, b.FECHA, lp.precio
                    UNION ALL'
                   SET @cSQL2= @cSQL2 +' SELECT  a.articulo, a.DEFINICION, SUM(a.unidades) as unidades,a.empresa, a.cliente, lp.precio
            ...
            SET @cSQL3= @cSQL + ' SELECT ISNULL((SELECT * FROM( ' + @cSQL2 + ') a
					group by a.articulo,a.DEFINICION, a.empresa,a.cliente, a.precio,A.unidades
					order by a.ARTICULO									
