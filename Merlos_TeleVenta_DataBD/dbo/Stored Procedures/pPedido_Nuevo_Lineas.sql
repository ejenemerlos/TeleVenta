...
BEGIN TRY 
	...
	begin try
		set @sentencia = '
			...
			select t.UserName
				...
				, case when t.articulo=''''
					then 0.00
					else case when cli.RECARGO=1 then 1 else 0 end
				  end as recargo
				, case when coalesce(art.pverde,0) =1 and coalesce(art.p_tan,0)=2 and cli.PVERDE=0 
					then case when t.peso>0
						...
						else t.unidades * coalesce(art.P_IMPORTE,0.00)
						end
					else
						0.00
				end as costeresiduo
			...
			inner join ['+@Gestion+'].dbo.clientes cli on cli.CODIGO=c.cliente collate Modern_Spanish_CS_AI
			left join ['+@Gestion+'].dbo.articulo art on art.CODIGO=t.articulo collate Modern_Spanish_CS_AI
			left join ['+@Gestion+'].dbo.tipo_iva iva on iva.CODIGO =  case when cli.tipo_iva!='''' then cli.TIPO_IVA else art.tipo_iva end
	...
	begin catch
		...
		BEGIN TRY
			...
			@recipients = 'e.jene@merlos.net;jc.villalobos@merlos.net;a.rodriguez@merlos.net',
			@body = @EMAILMENSAJE,
			@subject = @EMAILASUNTO
