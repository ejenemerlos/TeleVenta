...
' set @Sentencia = @Sentencia + '
	...
	*/
	set @consulta = ''
	WITH VENTAS (FECHA, CLIENTE, ARTICULO, DEFINICION, CAJAS, UDS, PESO, PVP, UNICAJA)
	AS
	(
		...
		INNER JOIN [''+@gestion+''].DBO.ARTICULO ART ON ART.CODIGO=DAV.ARTICULO
		INNER JOIN Configuracion_SQL CON ON 1=1
		INNER JOIN [''+@gestion+''].DBO.PVP PVP ON PVP.ARTICULO=DAV.ARTICULO AND PVP.TARIFA=''''TM''''
		WHERE CLI.FECHA_BAJ IS NULL AND ART.BAJA=0 AND CAV.FECHA>=GETDATE()-(30*CON.MesesConsumo) AND CAV.TRASPASADO=0
		AND CLI.CODIGO=''''''+@cliente+'''''' ''+@marcas+'' '' +@familias+ '' '' +@subfams+ ''
...
' set @Sentencia = @Sentencia + '
	...
	( 
		...
		, V1.CAJAS, V1.UDS, V1.PESO, V1.PVP, V1.UNICAJA
		...
		GROUP BY  V1.FECHA, V1.CLIENTE, V1.ARTICULO, V1.DEFINICION, V1.CAJAS, V1.UDS, V1.PESO, V1.PVP, V1.UNICAJA
