...
' set @Sentencia = @Sentencia + '
	...
	*/
	...
	(
		...
		FROM [2021YB].DBO.C_ALBVEN CAV 
		INNER JOIN [2021YB].DBO.D_ALBVEN DAV ON DAV.EMPRESA=CAV.EMPRESA AND DAV.LETRA=CAV.LETRA AND DAV.NUMERO=CAV.NUMERO
		INNER JOIN [2021YB].DBO.CLIENTES CLI ON CLI.CODIGO=CAV.CLIENTE
		INNER JOIN [2021YB].DBO.ARTICULO ART ON ART.CODIGO=DAV.ARTICULO
		INNER JOIN [TELEVENTA].DBO.[Configuracion_SQL] CON ON 1=1
		INNER JOIN [2021YB].DBO.PVP PVP ON PVP.ARTICULO=DAV.ARTICULO AND PVP.TARIFA=''''TM''''
		WHERE CLI.FECHA_BAJ IS NULL AND ART.BAJA=0 AND CAV.FECHA>=GETDATE()-(30*CON.MesesConsumo) AND CAV.TRASPASADO=0
		AND CLI.CODIGO=''''''+@cliente+'''''' ''+@marcas+'' '' +@familias+ '' '' +@subfams+ ''
		...
		SELECT CAV.FECHA, CAV.CLIENTE, DAV.ARTICULO, DAV.DEFINICION, DAV.CAJAS, DAV.UNIDADES, DAV.PESO, PVP.PVP 
		, ART.UNICAJA, ART.PESO as PesoArticulo
		FROM [2020YB].DBO.C_ALBVEN CAV 
		INNER JOIN [2020YB].DBO.D_ALBVEN DAV ON DAV.EMPRESA=CAV.EMPRESA AND DAV.LETRA=CAV.LETRA AND DAV.NUMERO=CAV.NUMERO
		INNER JOIN [2021YB].DBO.CLIENTES CLI ON CLI.CODIGO=CAV.CLIENTE
		INNER JOIN [2021YB].DBO.ARTICULO ART ON ART.CODIGO=DAV.ARTICULO
		INNER JOIN [TELEVENTA].DBO.[Configuracion_SQL] CON ON 1=1
		INNER JOIN [2021YB].DBO.PVP PVP ON PVP.ARTICULO=DAV.ARTICULO AND PVP.TARIFA=''''TM''''
	...
	( 
		...
		LEFT JOIN [TELEVENTA].DBO.vStock STC ON STC.ARTICULO=V1.ARTICULO 
