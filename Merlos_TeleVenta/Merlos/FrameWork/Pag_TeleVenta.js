...
function asignarMesesConsumo(){
	flexygo.nav.execProcess('pConfiguracion','',null,null, 
		[{ 'Key': 'modo', 'Value': 'MesesConsumo' }, { 'Key': 'valor', 'Value': $("#inpMesesConsumo").val() }], 'current', false, $(this), function (ret) {
			if (ret) { 
...
function cargarConfiguracionTV(elCallBack){
	...
	flexygo.nav.execProcess('pConfiguracion','',null,null,[{'Key':'parametros','Value':limpiarCadena(parametros)}],'current',false,$(this),function(ret){if(ret){ 
...
function cargarSubDatos(objeto,cliente,id,pos){
    ...
    flexygo.nav.execProcess('pObjetoDatos','',null,null,[{'Key':'elJS','Value':elJS}], 'current', false, $(this), function(ret){if(ret){
	...
	if(confirmacion){
		...
		flexygo.nav.execProcess('pMerlos','',null,null,[{'Key':'parametros','Value':limpiarCadena(parametros)}],'current',false,$(this),function(ret){if(ret){
			if(parseInt(ret.JSCode)===1){ setTimeout(function(){ contadorPedidoErrorReenviar++; PedidoErrorReenviar(IdTransaccion,confirmacion); },500); } 
				...
				flexygo.nav.execProcess('pMerlos','',null,null,[{'Key':'parametros','Value':limpiarCadena(parametros)}],'current',false,$(this),function(rt){if(rt){
					tbLlamadasGlobal_Sel(IdTeleVenta,FechaTeleVenta,NombreTeleVenta,1);				
...
function asignarserieconfig(){
	...
	flexygo.nav.execProcess('pConfiguracion','',null,null,[{'Key':'modo','Value':'TVSerie'},{'Key':'valor','Value':serie}],'current', false, $(this), function(ret){if(ret){
...
function inpDatos_Click(id){
	...
	flexygo.nav.execProcess('pInpDatos','',null,null,[{'Key':'elJS','Value':elJS}], 'current', false, $(this), function(ret){if(ret){
		var js = JSON.parse(limpiarCadena(ret.JSCode));		
		if(js.length>0){
			for(var i in js){ 
...
function cargarArticulosDisponibles(modo){
	...
	$("#spanPersEPinfo").fadeIn(300,function(){
		...
		flexygo.nav.execProcess(elSP,'',null,null,[{"Key":"parametros","Value":limpiarCadena(parametros)}],'current',false,$(this),function(ret){
			if(ret){
				...
				if(art.length>0){
					...
					for(var i in art){	
						...
						contenido += " 	<td class='C trBscCajas inv' style='width:10%;'>"+elPedUniCaja+"</td>"
									...
									+"			setTimeout(function(){ cargarPrecio("+i+",\"inp_cajas_"+i+"\",\"cajas\"); },300);}' "
									+"		onclick='$(this).select(); cargarPrecio("+i+",this.id,\"cajas\"); event.stopPropagation();' "+CajasIO+">"
									+"	</td>"
									+"	<td class='C' style='width:5%;'>"
									...
									+"			setTimeout(function(){ cargarPrecio("+i+",\"inp_uds_"+i+"\",\"unidades\"); },300);}' "
									+"		onclick='$(this).select(); cargarPrecio("+i+",this.id,\"uds\"); event.stopPropagation();' "+UdsIO+">"
									+"	</td>"
									+"	<td class='C' style='width:5%;'>"
									...
									+"			setTimeout(function(){ cargarPrecio("+i+",\"inp_peso_"+i+"\",\"peso\"); },300);}' "
									+"		onclick='$(this).select(); cargarPrecio("+i+",this.id,\"peso\"); event.stopPropagation();' "+PesoIO+">"
									+"	</td>";
					...
					if(esconder_spResBtn){$("#spResultadosT, #spResBtn").hide(); paginadorRegistros=0; }
					if(modo){ 
...
function PedidoEliminarLinea(jsId) {
	...
	flexygo.nav.execProcess('pPedido_Linea','',null,null,[{'key':'parametros','value':limpiarCadena(parametros)}],'current',false,$(this),function(r1){if(r1){
		pedidoVerDetalle(); 
...
function terminarLlamada(reintento){
	...
	if(PedidoDetalle.length>0 && PedidoGenerado===""){ 
		...
		flexygo.nav.execProcess('pMerlos','',null,null,[{'Key':'parametros','Value':limpiarCadena(parametros)}],'current',false,$(this),function(veu){if(veu){
...
function generarNuevoPedido(losParametros){ 
	...
	flexygo.nav.execProcess('pPedido_Nuevo','Pedido',null,null,losParametros,'current',false,$(this),function(ret){if(ret){ 
...
function ConfirmacionPedidoTratarVelo(){
	if(!$("#ConfirmacionPedidoBtnConfirmar").is(":visible")){ setTimeout(ConfirmacionPedidoTratarVelo,100); }
	else{
			...
			flexygo.nav.execProcess('pArticulos','',null,null,[{'Key':'parametros','Value':limpiarCadena(parametros)}],'current',false,$(this),function(ret){if(ret){
				...
				flexygo.nav.execProcess('pArticulos','',null,null,[{'key':'parametros','value':limpiarCadena(param)}],'current',false,$(this),function(r){if(r){
					...
					var p1 = '{"lineas":['+laLinea+'],"IdTeleVenta":"'+IdTeleVenta+'","IdTransaccion":"'+IdTransaccion+'"'
							...
							+','+paramStd+'}';
					flexygo.nav.execProcess('pPedido_Linea','',null,null,[{'key':'parametros','value':limpiarCadena(p1)}],'current',false,$(this),function(r1){if(r1){
